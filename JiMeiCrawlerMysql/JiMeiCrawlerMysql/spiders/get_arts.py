...
class Try(scrapy.Spider):
	...
	college_urls='http://arts.jmu.edu.cn/index/xyfc.htm'

	production_urls=['http://arts.jmu.edu.cn/wszt/xszp.htm',
	...
	def start_requests(self):
		...
		yield scrapy.Request(url=self.college_url,callback=self.parse_college)
		for production_url in self.production_urls:
			yield  scrapy.Request(url=production_url,callback=self.parse_production)
	...
	def parse_college(self,response):
		selector = scrapy.Selector(response)
		infos=selector.xpath('//td[re:test(@id, "id_\\d*")]//a//@href').extract() #得到每个菜单栏链接页面的所有info/*.html链接
		next=selector.xpath("//a[@class='Next']/@href").extract()
		if next:  # 假如有下一页继续爬取
			next=next[0]
			word="".join(response.url.split("/")[-1:])
	...
	def parse_production(self,response):
		selector = scrapy.Selector(response)
		infos=selector.xpath('//td[re:test(@id, "line_u4_\\d*")]//a//@href').extract() #得到每个菜单栏链接页面的所有info/*.html链接
		next=selector.xpath("//a[@class='Next']/@href").extract()
		if next:  # 假如有下一页继续爬取
			next=next[0]
			word="".join(response.url.split("/")[-1:])
